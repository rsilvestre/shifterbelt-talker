...
var ArrayKniffe = (function (_Array) {
  function ArrayKniffe() {
    _classCallCheck(this, ArrayKniffe);

    _get(Object.getPrototypeOf(ArrayKniffe.prototype), 'constructor', this).call(this);
  }

  _inherits(ArrayKniffe, _Array);

  _createClass(ArrayKniffe, [{
    key: 'inArray',
  ...
  _createClass(ExchangeService, [{
    ...
    value: function init() {
      ...
      this._messageInTmp.on('device_connect', function (deviceId) {
        var deviceList = new ArrayKniffe();
        ...
        var cleanEvent = function cleanEvent() {
          _this._messageInTmp.removeAllListeners('device_connect');
          deviceList.forEach(function (id) {
            localMessageIn.removeAllListeners(id);
          });
          localMessageOut.removeAllListeners('send');
          _this._messageInTmp.removeAllListeners('device_disconnect');
          _this._messageInTmp.removeAllListeners('device_connect');
          //localMessageIn = null;
        ...
        _this._messageInTmp.on('device_disconnect', function (deviceId) {
          //cleanEvent();
